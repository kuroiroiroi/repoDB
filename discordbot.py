...
def init():
	...
	try:
		...
		basicSetting.append(inputData[12][11:])     #basicSetting[2] : mungChk1
		basicSetting.append(inputData[11][16:])     #basicSetting[3] : before_alert1
		basicSetting.append(inputData[16][14:16])  #basicSetting[4] : restarttime 시
		basicSetting.append(inputData[16][17:])    #basicSetting[5] : restarttime 분
		...
		basicSetting.append(inputData[15][14:])    #basicSetting[11] : !q 표시 보스 수
		basicSetting.append(inputData[17][16:])    #basicSetting[12] : restart 주기
		basicSetting.append(inputData[6][17:])     #basicSetting[13] : 정산 채널 ID
		basicSetting.append(inputData[18][12:])    #basicSetting[14] : 스프레드시트 파일 이름
		basicSetting.append(inputData[19][11:])    #basicSetting[15] : json 파일명
		basicSetting.append(inputData[20][12:])    #basicSetting[16] : 시트 이름
		basicSetting.append(inputData[21][12:])    #basicSetting[17] : 입력 셀
		basicSetting.append(inputData[22][13:])    #basicSetting[18] : 출력 셀
		basicSetting.append(inputData[14][13:])    #basicSetting[19] : 멍삭제횟수
		...
		basicSetting.append(inputData[23][10:])     #basicSetting[23] : aws_key
		basicSetting.append(inputData[24][17:])     #basicSetting[24] : aws_secret_key
		basicSetting.append(inputData[25][12:])     #basicSetting[25] : voice_use
		basicSetting.append(inputData[13][11:])     #basicSetting[26] : mungChk2
	except:
	...
	for j in range(bossNum):
		...
		f.append(tmp_bossData[j][6][13:])		  #bossData[6] : 카톡On/Off		
			...
			for j in range(bossNum):
				...
				if beforeBossData[i+1][startPos+2:endPos] == bossData[j][0] :
					...
					if bossData[j][8] == "1":
						...
						tmp_now_chk = tmp_now + datetime.timedelta(minutes = int(basicSetting[26]))
...
def KakaoAlertSave(saveBossName, AlertStatus):
	...
	for value in indexBossname:
		if katalkData[value].find(saveBossName) != -1:
			katalkData[value + 6] = 'kakaoOnOff = '+ AlertStatus + '\n'
...
class taskCog(commands.Cog): 
	...
	async def boss_check(self):
		...
		while True:
			...
			tmp_aftr2 = now+datetime.timedelta(minutes=int(0-int(basicSetting[26])))
			...
			if channel != '':
				...
				############# 음성접속! ###########
				if len(self.bot.voice_clients) == 0 and self.checker and basicSetting[25] == "1":
					try:
						await self.bot.get_channel(basicSetting[6]).connect(reconnect=True, timeout=5)
						print(f"{now.strftime('%Y-%m-%d %H:%M:%S')} : 음성 채널 자동 재접속완료!")
					except discord.errors.ClientException as e:
						print(f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')} : 음성 자동 접속 부분에서 서버 음성 채널 이미 접속 에러 : {e}")
						self.checker = False
						pass
				...
				for i in range(bossNum):
					...
					if bossMungFlag[i] == True:
						...
						if (bossTime[i]+datetime.timedelta(days=-365)) <= aftr:
							if basicSetting[2] != '0' and basicSetting[26] != '0' and bossFlag[i] == True and bossFlag0[i] == True and bossMungFlag[i] == True :
								if int(basicSetting[19]) <= bossMungCnt[i] and int(basicSetting[19]) != 0:
									...
									bossMungCnt[i] = 0
									if bossData[i][2] == '0':
										await self.bot.get_channel(channel).send(f'```자동 미입력 횟수 {basicSetting[19]}회 초과! [{bossData[i][0]}] 삭제!```', tts=False)
										print ('자동미입력 횟수초과 <' + bossData[i][0] + ' 삭제완료>')
									else:
										await self.bot.get_channel(channel).send(f'```자동 멍처리 횟수 {basicSetting[19]}회 초과! [{bossData[i][0]}] 삭제!```', tts=False)
...
class mainCog(commands.Cog): 
	...
	async def join_(self, ctx):
		...
		if basicSetting[7] == "":
			...
			if basicSetting[6] != "" and basicSetting[25] == "1":
				try:
	...
	async def menu_(self, ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			command_list += ','.join(command[46]) + ' [추첨인원] (대기시간/초) *(메모)\n'    #!럭키박스
			...
			command_list += ','.join(command[45]) + ' 또는 ' + ', '.join(command[45]) + ' 0000, 00:00\n'     #!멍일괄
			command_list += ','.join(command[48]) + f' [00:00:00 : 보스명(엔터) ...]\n※ 보스탐 결과 복붙 가능\nex){command[48][0]} + 12:34:00 : {bossData[0][0]}\n+ 10:56:00 : {bossData[1][0]}\n+ (+1d) 12:12:00 : {bossData[2][0]}...\n'     #!컷등록
			command_list += ','.join(command[49]) + f' [00:00:00 : 보스명(엔터) ...]\n※ [00:00:00 보스명] 형태로 여러줄(엔터)로 구분하여 등록\nex){command[49][0]} + 12:34:00 : {bossData[0][0]}\n10:56:00 : {bossData[1][0]}\n+ (+1d) 12:12:00 : {bossData[2][0]}...\n'     #!예상등록
			command_list += ','.join(command[50]) + ' [시간(00:00)] [추가시간(숫자)] [보스명1] [보스명2] [보스명3] ...\n'     #!추가등록
			...
			command_list += ','.join(command[47]) + ' 사다리, 정산, 척살, 경주, 아이템\n'     #!채널삭제
	...
	async def setting_(self, ctx):	
		...
		if ctx.message.channel.id == basicSetting[7]:
			setting_val = '보탐봇버전 : PC Ver. 34 (2021. 6. 23.)\n'
			if basicSetting[6] != "" :
				setting_val += '음성채널 : ' + ctx.guild.get_channel(basicSetting[6]).name + '\n'
			setting_val += '텍스트채널 : ' + ctx.guild.get_channel(basicSetting[7]).name +'\n'
			if basicSetting[10] != "" :
				setting_val += '사다리채널 : ' + ctx.guild.get_channel(int(basicSetting[10])).name + '\n'
			if basicSetting[13] != "" :
				setting_val += '정산채널 : ' + ctx.guild.get_channel(int(basicSetting[13])).name + '\n'
			if basicSetting[20] != "" :
				setting_val += '척살채널 : ' + ctx.guild.get_channel(int(basicSetting[20])).name + '\n'
			if basicSetting[21] != "" :
				setting_val += '경주채널 : ' + ctx.guild.get_channel(int(basicSetting[21])).name + '\n'
			if basicSetting[22] != "" :
				setting_val += '아이템채널 : ' + ctx.guild.get_channel(int(basicSetting[22])).name + '\n'
			if basicSetting[8] != "" :
			...
			setting_val += '보스멍확인시간2 : ' + basicSetting[26] + ' 분 후\n'
			if basicSetting[25] == "0":
	...
	async def chMove_(self, ctx):	
		...
		if ctx.message.channel.id == basicSetting[7]:
			...
			print (f'[ {self.bot.get_channel(basicSetting[7]).name} ]에서')
			print (f'] {self.bot.get_channel(channel).name} [이동')
	...
	async def restart_(self, ctx):
		...
		if ctx.message.channel.id == basicSetting[7]:
			if basicSetting[2] != '0' and basicSetting[26] != '0':
				for i in range(bossNum):
					if bossMungFlag[i] == True:
	...
	async def ladder_(self, ctx : commands.Context, *, args : str = None):
		if basicSetting[10] != "" and ctx.message.channel.id == basicSetting[7]:
			return
	...
	async def jungsan_(self, ctx):
		if basicSetting[13] != "" and ctx.message.channel.id == basicSetting[7]:
			return
			...
			for i in range(bossNum):
				if bossTimeString[i] == '99:99:99' and bossMungFlag[i] != True :
					...
					if bossMungFlag[i] == True :
						...
						tmp_time_delta = (tmp_bossTime[i].date() - datetime.datetime.now().date()).days
						if tmp_time_delta == 0:
	...
	async def race_(self, ctx):
		if basicSetting[21] != "" and ctx.message.channel.id == basicSetting[7]:
			return
	...
	async def remove_channel_(self, ctx):
		...
		if msg == '사다리' : #사다리 채널 설정
			...
			ch_name = ctx.guild.get_channel(int(basicSetting[10]))
			inidata_text = open('test_setting.ini', 'w', encoding = 'utf-8')	
			for i in range(len(inputData_text)):
				if inputData_text[i].startswith('ladderchannel'):
					inputData_text[i] = 'ladderchannel = \n'
					basicSetting[10] = ""
			...
			ch_name = ctx.guild.get_channel(int(basicSetting[13]))
			inidata_text = open('test_setting.ini', 'w', encoding = 'utf-8')				
			for i in range(len(inputData_text)):
				if inputData_text[i].startswith('jungsanchannel'):
					inputData_text[i] = 'jungsanchannel = \n'
					basicSetting[13] = ""
			...
			ch_name = ctx.guild.get_channel(int(basicSetting[21]))
			inidata_text = open('test_setting.ini', 'w', encoding = 'utf-8')				
			for i in range(len(inputData_text)):
				if inputData_text[i].startswith('racingchannel'):
					inputData_text[i] = 'racingchannel = \n'
					basicSetting[21] = ""
			...
			ch_name = ctx.guild.get_channel(int(basicSetting[22]))		
			inidata_text = open('test_setting.ini', 'w', encoding = 'utf-8')				
			for i in range(len(inputData_text)):
				if inputData_text[i].startswith('itemchannel'):
					inputData_text[i] = 'itemchannel = \n'
					basicSetting[22] = ""
	...
	async def itemInit_(self, ctx):
		if basicSetting[22] != "" and ctx.message.channel.id == basicSetting[7]:
			return
	...
	async def command_voice_use(self, ctx : commands.Context):
		...
		for i in range(len(inputData_text)):
			if inputData_text[i].startswith("voice_use ="):
				inputData_text[i] = f"voice_use = 1\n"
				basicSetting[25] = "1"
	...
	async def command_randombox_game(self, ctx : commands.Context, *, args : str = None):
		...
		embed = discord.Embed(title  = f"📦 럭키박스! 묻고 더블로 가! (잔여시간 : {waiting_time}초)", description = f"참가를 원하시면 ✅를 클릭해주세요!", timestamp = datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=0))),
			color=0x00ff00
			)
		if memo_data != "":
	...
	async def multi_boss_cut(self, ctx, *, args : str = None):
		...
		for i in range(bossNum):
			if bossData[i][0] in boss_data_dict:
				...
				await dbSave()
		return await ctx.send(f"```[{', '.join(result_boss_name)}] 보스 [컷등록]이 완료되었습니다. [{command[27][0]}]으로 등록시간을 확인해보세요```", tts=False)
	...
	async def multi_boss_predict(self, ctx, *, args : str = None):
		...
		for i in range(bossNum):
			if bossData[i][0] in boss_data_dict:
				...
				await dbSave()
		return await ctx.send(f"```[{', '.join(result_boss_name)}] 보스 [예상등록]이 완료되었습니다. [{command[27][0]}]으로 등록시간을 확인해보세요```", tts=False)
	...
	async def multi_boss_delta_add(self, ctx, *, args : str = None):
		...
		if not args:
			return await ctx.send(f"```[{command[50][0]}] [시간(00:00)] [추가시간(숫자)] [보스명1] [보스명2] [보스명3] ... 양식으로 입력해주세요```", tts=False)
...
class IlsangDistributionBot(commands.AutoShardedBot):
	...
	async def on_ready(self):
		...
		if str(basicSetting[7]) in channel_id:
			...
			if basicSetting[25] == "1" and str(basicSetting[6]) in channel_voice_id:
				try:
					...
					pass				
			elif basicSetting[25] == "1" and str(basicSetting[6]) not in channel_voice_id:
				print(f"설정된 음성채널 값이 없거나 잘못 됐습니다. 음성채널 접속 후 **[{command[6][0]}]** 명령어 먼저 입력하여 사용해주시기 바랍니다.")
				await self.get_channel(int(basicSetting[7])).send(f"설정된 음성채널 값이 없거나 잘못 됐습니다. 음성채널 접속 후 **[{command[6][0]}]** 명령어 먼저 입력하여 사용해주시기 바랍니다.")
			if basicSetting[10] != "":
				if str(basicSetting[10]) in channel_id:
					print('< 사다리채널 [' + self.get_channel(int(basicSetting[10])).name + '] 접속완료 >')
				else:
					basicSetting[10] = ""
					print(f"사다리채널 ID 오류! [{command[33][0]} 사다리] 명령으로 재설정 바랍니다.")
			if basicSetting[13] != "":
				if str(basicSetting[13]) in channel_id:
					print('< 정산채널 [' + self.get_channel(int(basicSetting[13])).name + '] 접속완료>')
				else:
					basicSetting[13] = ""
					print(f"정산채널 ID 오류! [{command[33][0]} 정산] 명령으로 재설정 바랍니다.")
			if basicSetting[20] != "":
				if str(basicSetting[20]) in channel_id:
					print('< 척살채널 [' + self.get_channel(int(basicSetting[20])).name + '] 접속완료>')
				else:
					basicSetting[20] = ""
					print(f"척살채널 ID 오류! [{command[33][0]} 척살] 명령으로 재설정 바랍니다.")
			if basicSetting[21] != "":
				if str(basicSetting[21]) in channel_id:
					print('< 경주채널 [' + self.get_channel(int(basicSetting[21])).name + '] 접속완료>')
				else:
					basicSetting[21] = ""
					print(f"경주채널 ID 오류! [{command[33][0]} 경주] 명령으로 재설정 바랍니다.")
			if basicSetting[22] != "":
				if str(basicSetting[22]) in channel_id:
					print('< 아이템채널 [' + self.get_channel(int(basicSetting[22])).name + '] 접속완료>')
				else:
					basicSetting[22] = ""
					print(f"아이템채널 ID 오류! [{command[33][0]} 아이템] 명령으로 재설정 바랍니다.")
			if int(basicSetting[12]) != 0 :
	...
	async def on_message(self, msg):
		...
		if chflg == 1 :
			if self.get_channel(basicSetting[7]).id == msg.channel.id:
				...
				for i in range(bossNum):
					################ 보스 컷처리 ################ 
					# if message.content.startswith(bossData[i][0] +'カット'):
					if message.content.startswith(bossData[i][0] +'カット') or message.content.startswith(convertToInitialLetters(bossData[i][0] +'カット')) or message.content.startswith(bossData[i][0] +'カット') or message.content.startswith(convertToInitialLetters(bossData[i][0] +'カット')):
						if hello.find('  ') != -1 :
					...
					if message.content.startswith(bossData[i][0] +'예상') or message.content.startswith(bossData[i][0] +' 예상'):
						...
						if len(hello) > len(tmp_msg) + 4 :
							...
							await self.get_channel(channel).send('```' + bossData[i][0] + ' 예상 시간을 입력해주세요.```')
					...
					if message.content.startswith(bossData[i][0] +'메모 '):
						...
						await self.get_channel(channel).send('< ' + bossData[i][0] + ' [ ' + bossData[i][7] + ' ] 메모등록 완료>', tts=False)
					...
					if message.content.startswith(bossData[i][0] +'카톡끔'):
						...
						await self.get_channel(channel).send('< ' + bossData[i][0] + ' 카톡 보내기 끔>', tts=False)

					if message.content.startswith(bossData[i][0] +'카톡켬'):
						bossData[i][6] = '1'
						KakaoAlertSave(bossData[i][0], bossData[i][6])
						await self.get_channel(channel).send('< ' + bossData[i][0] + ' 카톡 보내기 켬>', tts=False)
